...
const PKG_MANAGERS = ["npm", "yarn", "bun", "pnpm"] as const;

export type PkgManager = (typeof PKG_MANAGERS)[number];

function craFlagsForTemplate(templateId: TemplateId) {
  switch (templateId) {
...
export function craTemplateCommandTabs(templateId: TemplateId, channel = "canary") {
...
function devCommand(pkgManager: PkgManager) {
  switch (pkgManager) {
    case "npm":
      return "npm run dev";
    case "yarn":
...
export function craTemplateWorkflowTabs(templateId: TemplateId, channel = "canary") {
  return PKG_MANAGERS.map((name) => ({
    name,
    code: [
