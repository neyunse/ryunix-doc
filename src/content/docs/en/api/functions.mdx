...
| Export | Purpose |
| :--- | :--- |
| `Link` | Declarative client navigation |
| `usePathname()` | Current path without query |
| `useSearchParams()` | URL search params object |
| `useRouter()` | Full router (`params`, `query`, navigation) |
...
| `useDebounce(value, delay)` | Built-in debounced value |
| `useThrottle(fn, delay)` | Throttled callbacks |
| `usePersistentStore(key, initial)` | State persisted to storage |

## Metadata helpers

| Export | Purpose |
| :--- | :--- |
| `mergeRouteMetadata(base, next)` | Layout → page merge preserving title templates |
| `resolvePageMetadata(meta, options?)` | Normalize title + flat tags for `<head>` injection |
...
1. Name custom hooks with the `use` prefix
...
- [SEO & Metadata](/en/docs/guides/seo-metadata) — `useMetadata` and `Metatags`
